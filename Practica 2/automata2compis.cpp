...
//                                                 0    la cadena #estados
int explorar(Estado estadoActual,Estado *estados,int it,char * cad,int i,char * F,std::string loQueSeImprime,std::string loQueNoSeImprime){
  ...
  if(vsz==0 && tamvic==0) {
    ...
    for(int k=0;k<tamvic;k++){//con esto itero en el vector para epsilon
      for(int j=0;j<loQueNoSeImprime.size();j++){//este pedazo de codigo horrible sirve para encontrar loops de epsilon
        for(int l=0;l<loQueNoSeImprime.size();l++){
          if((j!=l)&&(loQueNoSeImprime[j]==loQueNoSeImprime[l])){
            return 0;}}}
      if((it == strlen(cad)-1 && victor[k]==F[0]) ||(it == strlen(cad)-1 && victor[k]==F[2])||(it == strlen(cad)-1 && victor[k]==F[4])){
        loQueSeImprime.push_back(victor[k]);
        //loQueSeImprime.push_back(',');
        std::cout << loQueSeImprime <<"    sirve"<<'\n';
        }
      //std::cout << loQueSeImprime <<"    probandoo"<<'\n';
      int o = explorar(cambioEstado(estadoActual,estados,i,victor[k]),estados,it,cad,i,F,loQueSeImprime,loQueNoSeImprime);
      if(o == 0) { continue; }
    }
